        ...
        for dirpath, _, filenames in os.walk(self.path):
            for file in filenames:
                ...
                if not (fileExtension == ".cv" or fileExtension == ".hist" or fileExtension == ".sa" or fileExtension == ".dat" or fileExtension == ".npy" or fileExtension == ".txt"):
                    ...
                    tangentList = self.divider.divide(Tangent, self.column, self.row, option = self.divisionOption)
                    
                    MagnitudeList = self.divider2.divide(Magnitude, self.column, self.row, option = self.divisionOption)
...
if __name__ == "__main__":
    ...
    row = 5
    Parallel = True
    main = Main_pro(path ,visualize = True, writing_method = 'cv', division = "pro")
