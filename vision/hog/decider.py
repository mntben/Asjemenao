...
class Decider(AbstractVisionModule):
    ...
    def value_iteration(self):
        ...
        timestart = time.time()
        sum = 1000
        while sum > self.stopping_criterion:
            ...
            for i in range1:
                ...
                for j in range2:
                    ...
                    #If action was manually selected, give a lot of reward
                    if j == 4 or j == 5 or j == 6:
                        #print "setting high manual reward"
                        self.reward = 0
                    else:
                        self.reward = 0

                    for z in range1:
                        temp1 += self.tp_matrix[i,z,j]*(self.reward + self.discountValue*self.values[z])
